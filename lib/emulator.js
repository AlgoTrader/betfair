...
exports.enableBetEmulatorForMarket = function(marketId) {
    log && log.info('Enable emulator for market:'+marketId);
    core.enableBetEmulatorForMarket(marketId);
};

exports.disableBetEmulatorForMarket = function(marketId) {
    log && log.info('Disable emulator for market:'+marketId);
...
exports.startEmulatorLog = function(filename, level) {
    ...
    log.info('Emulator log started');
}

exports.stopEmulatorLog = function() {
    log && log.info('Emulator log stopped');
    log = null;
}
...
exports.onListMarketBook = function (result) {
    log && log.debug({result:result}, 'onListMarketBook');
    return core.onListMarketBook(result);
};

function request(httpOptions, cb) {
    log && log.debug({httpOptions: httpOptions}, 'New request for emulator');
    ...
    req.uuid = reqUuid;
    res.uuid = reqUuid;
    log && log.debug({httpOptions: httpOptions}, 'Request id:%s', reqUuid);
...
EmulatorRequest.prototype.end = function() {
    ...
    if(handler && typeof(handler)==='function') {
        log && log.debug('Call handler for method:'+self.method);
        handler(self, self.response, function(err, res) {
            sendResponse();
        });
    }
}
