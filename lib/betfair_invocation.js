...
// Endpoints
var apiTypes = {
    accounts: {type:"AccountAPING", version:"v1.0", service:"https://beta-api.betfair.com:443/account/json-rpc"},
    sports: {type:"SportsAPING", version:"v1.0", service:"https://beta-api.betfair.com:443/betting/json-rpc"},
};
...
// logging stuff
var log = null;
exports.startInvocationLog = function(filename, level) {
    ...
    log.info('Invocation log started');
}

exports.stopInvocationLog = function() {
    log.info('Invocation log stopped');
    log = null;
}
...
BetfairInvocation.prototype.execute = function (callback) {
    ...
    var httpOptions = {
        ...
        forever: true,
        headers: {
            'X-Application': self.applicationKey,
...
BetfairInvocation.prototype.end = function () {
    ...
    if(log) {
        ...
        log.info("method:%s, duration:%s, isSuccess:%s isEmulated:%s", 
            self.method, self.duration/1000, self.isSuccess, self.isEmulated);
        // full request and response
        log.debug({request:self.request, response:self.response}, self.method);
    }
